...
public:
    ...
    Polar operator+(Polar other) {
        float x = radius * cos(angle * 3.14159 / 180) + other.radius * cos(other.angle * 3.14159 / 180);
        float y = radius * sin(angle * 3.14159 / 180) + other.radius * sin(other.angle * 3.14159 / 180);
...
class Currency {
    int rupees, paise;
    void convert() { rupees += paise / 100; paise %= 100; }
public:
    Currency(int r = 0, int p = 0) : rupees(r), paise(p) { convert(); }

    Currency operator+(Currency o) { return Currency(rupees + o.rupees, paise + o.paise); }
    Currency operator-(Currency o) { return Currency(0, (rupees * 100 + paise) - (o.rupees * 100 + o.paise)); }
    Currency operator*(Currency o) { return Currency(0, (rupees * 100 + paise) * (o.rupees * 100 + o.paise) / 100); }
    bool operator==(Currency o) { return rupees == o.rupees && paise == o.paise; }
...
public:
    ...
    Temperature operator+(Temperature o) { return Temperature(celsius + o.celsius); }
...
int main() {
    ...
    float fahrenheit = result; int roundedCelsius = result;
    cout << "Fahrenheit: " << fahrenheit << "\nRounded Celsius: " << roundedCelsius << endl;
    return 0;
}
\end{code}
...
public:
    Item(string n = "", float p = 0, int q = 0) : name(n), price(p), quantity(q) {}
    Item operator+(Item o) {
        return (name == o.name) ? Item(name, price, quantity + o.quantity) : *this;
    }
    float operator*() { return price * quantity; }
    void display() { cout << name << " " << price << " " << quantity << endl; }
};

int main() {
    Item a("Pen", 10, 3), b("Pen", 10, 5);
    Item total = a + b;
    cout << "Combined item: "; total.display();
...
public:
    ...
    Date operator++() {
        if (++day > daysInMonth()) { day = 1; if (++month > 12) { month = 1; year++; } }
        return *this;
    }
    int operator-(Date o) {
        return (year * 365 + month * 30 + day) - (o.year * 365 + o.month * 30 + o.day);
    }
...
int main() {
    ...
    cout << "Before increment: "; first.display();
    ++first;
    cout << "After increment: "; first.display();
...
int main() {
    ...
    cout << (a == b ? "Positions are equal" : "Positions are not equal");
    return 0;
}
\end{code}
...
public:
    Book(string t, int copies) : title(t), copiesAvailable(copies) {}
    Book operator--() { if (copiesAvailable > 0) copiesAvailable--; return *this; }
    Book operator++() { copiesAvailable++; return *this; }
    bool operator==(Book o) { return title == o.title; }
    void display() { cout << title << ": " << copiesAvailable << " copies" << endl; }
};

int main() {
    Book a("C++ Programming", 5), b("C++ Programming", 2);
    cout << "Before issuing: "; a.display();
    --a; cout << "After issuing: "; a.display();
    ++a; cout << "After returning: "; a.display();
    cout << (a == b ? "Books are the same" : "Books are different");
    return 0;
}
\end{code}
...
public:
    ...
    bool operator==(Polynomial o) {
        ...
        for (int i = 0; i <= degree; i++)
            if (coefficients[i] != o.coefficients[i]) return false;
        return true;
    }
...
int main() {
    ...
    Polynomial second(2);
    cout << (first == second ? "Polynomials are identical" : "Polynomials are different") << endl;
    cout << (first < second ? "First polynomial has smaller degree" : "First polynomial does not have smaller degree");
    return 0;
}
\end{code}
...
class Cart {
    string items[20]; int itemCount;
public:
    Cart() : itemCount(0) {}
    void addItem(string item) { items[itemCount++] = item; }
    Cart operator+(Cart o) {
        Cart result = *this;
        for (int i = 0; i < o.itemCount; i++) result.addItem(o.items[i]);
        return result;
    }
    string operator[](int i) { return items[i]; }
    void display() { for (int i = 0; i < itemCount; i++) cout << items[i] << endl; }
};

int main() {
    Cart a, b;
    a.addItem("Book"); a.addItem("Pen");
    b.addItem("Bag"); b.addItem("Notebook");
    Cart combined = a + b;
    cout << "Combined cart:" << endl; combined.display();
...
public:
    Fraction(int top = 0, int bottom = 1) : numerator(top), denominator(bottom) {}
